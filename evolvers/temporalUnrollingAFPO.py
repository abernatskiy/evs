...
class Evolver(BaseEvolver):
	...
	def _getNewIndividual(self):
		...
		if self.params.has_key('initialPopulationType') and self.params['initialPopulationType'] == 'sparse':
			...
			indiv.mutate()
		indiv.mutTimeIncrease = 0.
		indiv.mutTimeDecrease = 0.
		self.evaluationTime = self._getTime(0)
		return indiv

	def _getTime(self, age):
		return 2. + float(age)/10. if age<500 else 52.
...
#				print str(indiv) + ' score: ' + str(indiv.score) + ' age: ' + str(indiv.age)
				print str(indiv.id) + ' score: ' + str(indiv.score) + ' age: ' + str(indiv.age)
			print ''
...
#		self.printPopulation()
		...
		for indiv in paretoFront:
			indiv.age += 1
			indiv.evaluationTime = self._getTime(indiv.age)
