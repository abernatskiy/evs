...
class Evolver(EFFEvolver):
	def _findFitnessVariantsElite(self, curIndivs, fitnessVariant, bestError):
		...
		fitnessVariantElite = self.findParetoFrontManyObjectives([self.getCurrentErrorFunc(), self.getConnectionCostFunc()], population=curIndivs)
		print('{}: elite size is {}, subpopulation size {}, densities: {}'.format(fitnessVariant, len(fitnessVariantElite), numIndivs, sorted([ (self.getConnectionCostFunc()(indiv), self.getCurrentErrorFunc()(indiv)) for indiv in fitnessVariantElite ])))
