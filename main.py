...
indivParams = {'length': 3*3, 'precision': 4, 'mutationProbability': 0.03, 'mutationAmplitude': 0.1, 'connectionCost': 0.0}
# second argument is connection cost

evolParams = {'indivClass': Individual, 'populationSize': 30, 'printParetoFront': True, 'randomSeed': int(sys.argv[1]), 'genStopAfter': 10000, 'secondMinObj': lambda x: len(filter(lambda y: y!=0, x.values))}
